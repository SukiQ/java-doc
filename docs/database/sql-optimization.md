...
复用是空间不变的根本原因：InnoDB 以页为单位管理，删除产生的“空洞”只供本表复用，**不会收缩文件**——这是为了性能（频繁收缩/扩张文件代价大）。
:::
...
对应 SQL 优化点里的简版，此处展开：核心都是**让排序过滤在索引上完成**，把"扫描+丢弃"变成"定位+返回"。
:::
...
结论一句话：**数行数用 count(*)（官方优化最好），count(列) 是另一个语义（非 NULL 计数）别混用**；大表精确计数靠计数表，允许误差用估算。
:::
